...
var VodModel = {
    ...
    insertMoviesToCategories:function(){
        ...
        var resume_movies=[], favourite_movies=[];
        var that=this;
        var movies_map={};
        var saved_video_times=this.saved_video_times;
        ...
        for(var i=0;i<movies.length;i++){
            ...
            if(saved_video_times[movie[movie_id_key].toString()])// if movie id is in recently viewed movie ids
            {
                if(that.recent_insert_position==="before")
                    resume_movies.unshift(movie);
                else
        ...
        var sorted_resume_movies=[];
        var resume_movie_ids=Object.keys(this.saved_video_times);
        for(i=0; i<resume_movie_ids.length; i++){
            for(var j=resume_movies.length-1; j>=0; j--){
                if(resume_movies[j].stream_id==resume_movie_ids[i]){
                    sorted_resume_movies.push(resume_movies[j])
                    resume_movies.splice(j,1);
                    break;
                }
            }
        }
        recent_category.movies=sorted_resume_movies;
